...
const SECTIONS: Section[] = [
  {
    ...
    fields: [
      ...
      { key: "usrp_a_args",   label: "USRP A rfargs (dual — DL device)", flag: "-X", widget: "text",
        hint: "Dual-USRP mode: e.g. serial=<A_serial> — the clock is set by the Clock/time source dropdown above. Use Auto-detect to fill." },
      { key: "usrp_b_args",   label: "USRP B rfargs (dual — UL device)", flag: "-Z", widget: "text",
        hint: "Dual-USRP mode: e.g. serial=<B_serial> — the clock is set by the Clock/time source dropdown above. Use Auto-detect to fill." },
      { key: "decimate",      label: "Decimate", flag: "-Y" },
      { key: "cpu_affinity",  label: "CPU affinity mask (-1 disable)", flag: "-y" },
    ],
    selectOptions: {
      clock_source: [
        { value: "internal", label: "internal — onboard oscillator" },
        { value: "gpsdo",    label: "gpsdo — onboard GPSDO (GPS 10 MHz + 1 PPS)" },
        { value: "external", label: "external — REF IN 10 MHz + PPS IN 1 PPS (shared)" },
      ],
    },
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "pcap_forward_host", label: "Destination IP / host", flag: "", widget: "text",
        hint: "The appliance connects OUT to this address and pushes the stream. This is the decrypt instance's IP — its Config page shows the exact IP:port to use and a live 'listening/connected' status." },
      { key: "pcap_forward_port", label: "Destination port", flag: "",
        hint: "Must match the decrypt instance's listen port (its Config page, default 9000) — not that instance's admin GUI port." },
      { key: "pcap_forward_compress", label: "Compress the stream (zstd, binary)", flag: "",
        hint: "On (default): zstd-compressed binary stream (~2.5× smaller on the wire). Off: raw libpcap. Either way the receiver auto-detects which one you're sending — no matching setting needed on that side." },
    ],
  },
  {
    title: "GUI",
